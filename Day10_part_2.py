...
TOT=0;
with open("Day10_input1_1.txt") as f:
    for kk_machine,r in enumerate(f.readlines()):
        ...
        Wanted_State=np.array([int(j) for j in joltage_list],dtype=np.uint16);
        print("Joltage length= ",Wanted_State.shape)
        print(Wanted_State)
        # Filling adjacency matrix

        Comandi=np.zeros((len(r[1:-1]),Wanted_State.shape[0]),dtype=np.uint16);
        ...
        #INIZIO CON DJIKSTRA
        Coda=[]
        Coda_di_qualsiasi_che_porta_a_smallest_digit=[]
        idx_smallest_digit=np.argmin(Wanted_State)
        Wanted_State_STR=str(Wanted_State)
        print("wanted state STR",Wanted_State_STR)
        SRC=np.array([0]*Wanted_State.shape[0],dtype=np.uint16);
        min_dist_to_dest=np.inf;
        heapq.heappush(Coda,(0,str(SRC))); # 100 by default come dist from wanted
        min_dist_to_node={str(SRC):0};

        while len(Coda)>0:
            actual_dist,nodo_actual_str=heapq.heappop(Coda)
            print(nodo_actual_str)
            nodo_actual=np.fromstring(nodo_actual_str[1:-1],dtype=np.uint16,sep=" ")
            if nodo_actual[idx_smallest_digit]>Wanted_State[idx_smallest_digit]:
                continue;
            if nodo_actual[idx_smallest_digit]==Wanted_State[idx_smallest_digit]:
                heapq.heappush(Coda_di_qualsiasi_che_porta_a_smallest_digit,(actual_dist,nodo_actual_str));
            if actual_dist>=min_dist_to_dest or actual_dist>min_dist_to_node[nodo_actual_str]:
                continue;
            #print("At dist ",actual_dist,"  at node ", nodo_actual)            
            for comm in Comandi:
                nodo_dest=nodo_actual+comm
                somma=actual_dist+1
                stri_nodo_dest=str(nodo_dest);
                if stri_nodo_dest not in min_dist_to_node.keys():
                    min_dist_to_node[stri_nodo_dest]=somma
                elif somma<min_dist_to_node[stri_nodo_dest]:
                    min_dist_to_node[stri_nodo_dest]=somma;
                
                if somma<min_dist_to_dest:
                    heapq.heappush(Coda,(somma,str(nodo_dest)))
            while len(Coda_di_qualsiasi_che_porta_a_smallest_digit)>0:
                actual_dist,nodo_actual_str=heapq.heappop(Coda_di_qualsiasi_che_porta_a_smallest_digit)
                #nodo_actual=np.fromstring(nodo_actual_str[1:-1],dtype=np.uint16,sep=" ")
                if nodo_actual_str==Wanted_State_STR:
