...
with open("Day11_input1.txt") as f:
    ...
    while len(stack)>0:
        nodo_actual,trovato_DAC,trovato_FFT=stack.pop()
        #print(nodo_actual)
        ...
        if nodo_actual==DEST:
            Value_nodes[DEST]=1
            #print("****")
            Value_nodes[nodi_visitati_questa_ricerca]=False
            if trovato_FFT and trovato_DAC:
                TOT+=1

            nodi_visitati_questa_ricerca=[];
            continue;
        elif Value_nodes[nodo_actual]:
            continue; # l' avevo già visitato questo nodo in questa ricerca
        else:
            nodi_visitati_questa_ricerca.append(nodo_actual);
            Value_nodes[nodo_actual]=True; # visitato
        locco=np.where(A[nodo_actual,:]!=0)[0]
        for nodo_dest in locco:
            stack.append((int(nodo_dest),trovato_DAC,trovato_FFT))
print(f"SOMMA ={Value_nodes[DEST]}" )