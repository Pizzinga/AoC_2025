...
with open("Day9_input1.txt") as f:
    ...
    for i in range(N):
        ...
        if l>BASE_MAX:
            ...
            i_max_lista=[i];
        if l==BASE_MAX:
            i_max_lista.append(i)
        if count_stessa_dire>MAX_stessa_dire:
            MAX_stessa_dire=count_stessa_dire+0;
    print("fine border creation")
    MAX=0;
    X=X.astype(np.float64)
    for i_max in i_max_lista:
        for i_row in [(i_max-kk)%N for kk in range(-(MAX_stessa_dire+2),MAX_stessa_dire+4,1)]:
            for j_col in [(i_max-kk)%N for kk in range(-(MAX_stessa_dire+2),MAX_stessa_dire+4,1)]:
                ...
                if Area>MAX: #and np.all(Mappa[min_x:max_x,min_y:max_y]):
                    ...
                    if valid_area(Bordo,min_x,max_x,min_y,max_y):#valid_area_v2(Bordo,min_x,max_x,min_y,max_y):
                        MAX=Area;
