...
with open("Day8_input1.txt") as f:
    ...
    joined_groups=[];
    for ind in sorted_indexes[:1000]:
    ...
    lengo=[];
    for k in range(len(joined_groups)):
        if len(joined_groups[k])!=0:        
            lengo.append(len(joined_groups[k]));
    
    lengo=sorted(lengo,reverse=True); 
    M=lengo[0]*lengo[1]*lengo[2]
    print(f"the final result of multiplication is {M}");